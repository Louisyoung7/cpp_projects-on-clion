...
void func(const st_girl &stgirl) {
    ...
    string sc = stgirl.sc;
    bool yz = stgirl.yz;
    if (age >= 18 && age <= 25 && height >= 165 && height <= 178 && sc == "身材火辣" && yz == true) {
        cout << "妃子" << endl;
    }
    else if (age >= 18 && age <= 30 && height >= 160 && height <= 165 && sc == "身材火辣或者普通" && yz == true) {
        cout << "宫女" << endl;
    }
    else if (age >= 35 && age <= 40 && height >= 155 && height <= 165 && sc == "身材普通或者飞机场" && yz == false) {
        cout << "嬷嬷" << endl;
    }
}